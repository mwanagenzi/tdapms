<x-layouts::app :title="__('Add Unit')">
    <flux:main class="mx-auto w-1/2 space-y-6">
        ...
        <form method="POST" action="{{ route('units.store') }}" class="space-y-5">
            ...
            <flux:field>
                ...
                <flux:select name="property_id" required>
                    ...
                    @foreach ($properties as $property)
                        <option value="{{ $property->id }}"  @selected(old('property_id', request('property_id')) == $property->id)>{{ $property->name }}</option>
                    @endforeach
                </flux:select>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <flux:field>
                    <flux:label aside="{{ __('Optional') }}">{{ __('Floor') }} </flux:label>
                    <flux:input name="floor" value="{{ old('floor') }}" placeholder="e.g. 2nd" />
                </flux:field>
                <flux:field>
                    <flux:label aside="{{ __('Optional') }}">{{ __('Size (sq ft)') }} </flux:label>
